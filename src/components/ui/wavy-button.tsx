...
interface WavyButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    children: React.ReactNode;
}

export function WavyButton({ children, className, ...props }: WavyButtonProps) {
    return (
        <button
            style={{ WebkitBoxReflect: "below 0px linear-gradient(to bottom, rgba(0,0,0,0.0), rgba(0,0,0,0.4))" }}
            className={cn(
                "px-10 py-3 w-full bg-gradient-to-r from-primary-container to-primary rounded-full shadow-xl group-hover:shadow-2xl group-hover:shadow-primary/50 shadow-primary/50 relative overflow-hidden group text-transparent cursor-pointer z-10 after:absolute after:rounded-full after:bg-primary/20 after:h-[85%] after:w-[95%] after:left-1/2 after:top-1/2 after:-translate-x-1/2 after:-translate-y-1/2 hover:saturate-[1.15] active:saturate-[1.4] h-16",
                className
            )}
            {...props}
        >
            <span
                className="absolute z-40 font-semibold bg-gradient-to-r from-on-primary-container to-white bg-clip-text text-transparent top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-full w-full flex items-center justify-center font-headline text-sm uppercase tracking-wider"
            >
                {children}
            </span>
            <svg
            ...
            >
                <defs>
                    <linearGradient id="sssurf-grad-purple" y2="100%" x2="50%" y1="0%" x1="50%">
                        <stop offset="0%" stopOpacity="1" stopColor="hsl(var(--primary))" />
                        <stop offset="100%" stopOpacity="1" stopColor="hsl(var(--primary-container))" />
                    </linearGradient>
                </defs>
                <g transform="matrix(1,0,0,1,0,-91.0877685546875)" fill="url(#sssurf-grad-purple)">
            ...
            <svg
                className="absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-[30%] group-hover:-translate-y-[33%] group-hover:scale-95 transition-all duration-500 z-40 fill-primary-container"
